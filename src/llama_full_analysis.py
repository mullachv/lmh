#!/usr/bin/env python3
"""
Full LLaMA Analysis for Manifold Distance Project
Runs comprehensive analysis on 130 diverse prompts using LLaMA-2-7b-hf
"""
...
def extract_llama_embeddings(prompts, model_name="meta-llama/Llama-2-7b-hf", max_prompts=None):
...
def analyze_llama_embeddings(embeddings, prompts):
    """Comprehensive analysis of LLaMA embeddings"""
    print(f"\n{'='*60}")
    ...
    # GMM clustering (with regularization for high-dimensional data)
    ...
    for k in range(2, min(6, len(prompts)//2)):
        try:
            ...
            gmm_labels = gmm.fit_predict(embeddings.astype(np.float64))
            
            if len(set(gmm_labels)) > 1:
                silhouette = silhouette_score(embeddings, gmm_labels)
                print(f"  GMM (k={k}): Silhouette = {silhouette:.3f}")
                
                if silhouette > best_gmm_silhouette:
                    best_gmm_silhouette = silhouette
                    best_gmm_k = k
        except Exception as e:
            print(f"  GMM (k={k}): Failed - {str(e)[:50]}...")
    
    print(f"  Best GMM: k={best_gmm_k}, Silhouette={best_gmm_silhouette:.3f}")
    ...
    return {
        ...
        'best_gmm': {'k': best_gmm_k, 'silhouette': best_gmm_silhouette}
    }

def visualize_llama_results(embeddings, prompts, analysis_results):
    """Create visualizations for LLaMA embeddings"""
    print(f"\n{'='*60}")
    ...
    scatter = axes[0, 2].scatter(embedding_2d[:, 0], embedding_2d[:, 1], 
                               c=kmeans_labels, cmap='tab10', s=20, alpha=0.7)
    axes[0, 2].set_title(f'LLaMA: K-means (k={analysis_results["best_kmeans"]["k"]})')
    axes[0, 2].set_xlabel('UMAP 1')
    axes[0, 2].set_ylabel('UMAP 2')
    
    # 4. DBSCAN clustering
    dbscan_labels = analysis_results['dbscan']['labels']
    ...
    for k, col in zip(unique_labels, colors):
        ...
        axes[1, 0].scatter(xy[:, 0], xy[:, 1], c=[col], marker=marker, s=size, alpha=alpha)
    
    axes[1, 0].set_title(f'LLaMA: DBSCAN ({analysis_results["dbscan"]["n_clusters"]} clusters, {analysis_results["dbscan"]["n_noise"]} noise)')
    axes[1, 0].set_xlabel('UMAP 1')
    axes[1, 0].set_ylabel('UMAP 2')
    
    # 5. Best GMM clustering
    gmm = GaussianMixture(n_components=analysis_results['best_gmm']['k'], random_state=42)
    ...
    axes[1, 1].set_title(f'LLaMA: GMM (k={analysis_results["best_gmm"]["k"]})')
    ...
    axes[1, 2].hist(distances, bins=30, alpha=0.7, edgecolor='black')
    axes[1, 2].set_title('LLaMA: Cosine Distance Distribution')
    axes[1, 2].set_xlabel('Cosine Distance')
    axes[1, 2].set_ylabel('Frequency')
    axes[1, 2].axvline(np.mean(distances), color='red', linestyle='--', 
                      label=f'Mean: {np.mean(distances):.3f}')
    axes[1, 2].legend()
    
    plt.tight_layout()
    plt.savefig('../data/llama_full_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    return embedding_2d, kmeans_labels, gmm_labels

def compare_with_other_models(llama_results):
    """Compare LLaMA results with other models"""
    print(f"\n{'='*60}")
...
def main():
    ...
    embeddings, prompts = extract_llama_embeddings(prompts, max_prompts=50)  # Start with 50 for testing
    
    # Analyze embeddings
    analysis_results = analyze_llama_embeddings(embeddings, prompts)
    
    # Create visualizations
    embedding_2d, kmeans_labels, gmm_labels = visualize_llama_results(embeddings, prompts, analysis_results)
    ...
    with open('../data/llama_full_analysis.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    print(f"\n✅ LLaMA analysis complete!")
    print(f"Results saved to ../data/llama_full_analysis.json")
    print(f"Visualization saved to ../data/llama_full_analysis.png")
